...
const sendConfirmationEmail = (name, email, confirmationCode) => {
  ...
  transport
    .sendMail({
      ...
      html:
        `<h1>Rettiwt Signup Email Confirmation</h1>
          <h2>Hello ${name}</h2>
        ...
        `/confirm?confirmationCode=${confirmationCode}> Click here</a>
          </div>`,
    })
...
const sendPasswordResetEmail = (name, email, confirmationCode) => {
  ...
  transport
    .sendMail({
      ...
      html:
        `<h1>Rettiwt Password Reset Request</h1>
                <h2>Hello ${name}</h2>
                ...
                <a herf =` +
        process.env.DEV_SERVER_PATH +
        `/reset-password?confirmationCode=${confirmationCode} Click here</a>
